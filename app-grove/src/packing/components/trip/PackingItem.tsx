...
export default function PackingItem({ item, onToggle, onDelete, onRename }: Props) {
  ...
  function handleTouchMove(e: React.TouchEvent) {
    ...
    if (swiped) {
      ...
      setOffsetX(-ACTION_WIDTH + newOffset + ACTION_WIDTH);
    ...
    <div className="relative overflow-hidden">
      ...
      {(offsetX !== 0 || swiped) && (
        <div className="absolute right-0 top-0 bottom-0 flex items-stretch" style={{ width: ACTION_WIDTH }}>
          <button
            onClick={() => { closeSwipe(); setEditing(true); setEditValue(item.name); }}
      ...
      >
        <button
          onClick={(e) => { if (swiped) { e.stopPropagation(); return; } onToggle(); }}
        ...
        >
          {item.checked && (
            <svg className="w-4 h-4 text-on-primary" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor">
        ...
        {editing ? (
          ...
          <span
            onClick={(e) => { if (swiped) { e.stopPropagation(); return; } onToggle(); }}
            className={`flex-1 text-sm cursor-pointer select-none ${item.checked ? 'line-through text-muted' : 'text-body'}`}
          >
            {item.name}
          </span>
        )}
      ...
      {contextMenu && (
        ...
        >
          <button
            onClick={() => { closeContextMenu(); setEditing(true); setEditValue(item.name); }}
